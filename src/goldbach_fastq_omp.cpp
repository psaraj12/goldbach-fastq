...
// v2.0.1 — Continuity-driven FastQ Goldbach engine
//
// Patch release:
// - Fixed last-winner continuity preservation
// - Stabilized FastQ learning order
// - Performance tuning without heuristic expansion
// ========================================================================================================================================================
...
static constexpr size_t FASTQ_RING_CAP = 16384;
static constexpr size_t FASTQ_WIN_CAP  = 8192;

static constexpr size_t FASTQ_SCAN0 = 64;
...
struct FastQ {
    ...
    bool was_seen = seen(p);

    // First-time learning: grow pool / ring only
    if (!was_seen) {
        pool.push_back(p);
        ring.push_back(p);
        if (ring.size() > FASTQ_RING_CAP)
            ring.pop_front();
        return;   // do NOT pollute win[]
    }

    // Second+ time: promote to hot window
    win[win_pos] = p;
...
bool scan(size_t k, F&& f) {
    // ---- 1) HOT WINDOW SCAN (small & tight) ----
    size_t wlim = min(win_cnt, (size_t)FASTQ_SCAN0); // e.g. 32 or 64
    size_t pos = (win_pos + FASTQ_WIN_CAP - 1) & (FASTQ_WIN_CAP - 1);

    for (size_t i = 0; i < wlim; ++i) {
        if (f(win[pos])) return true;
        pos = (pos - 1) & (FASTQ_WIN_CAP - 1);
    }

    // ---- 2) POOL TAIL SCAN (colder) ----
    if (pool.empty()) return false;

    size_t plim = min(k, pool.size());
    size_t i = pool.size();
...
void run(u64 startN, u64 endN, int threads) {
    ...
    SIEVE_SCAN_LIMIT  = min<u64>(16'384,SMALL_SIEVE_LIMIT);
...
if (have_last) {
    ...
    if ((p & 1) && p < N) {
        ...
        if (ok) {
           ...
           if (!found && fq.scan(FASTQ_SCAN2, try_p))
    found = true;
